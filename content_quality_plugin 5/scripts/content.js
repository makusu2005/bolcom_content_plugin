...
var video = document.querySelector('[data-test="product-video-thumb-1"]');
var images = document.querySelectorAll('.filmstrip__flex');

let review_score = 0;
let no_reviews = 0;

// average rating
const ratingEl = document.querySelector('.pdp-header__rating .text-neutral-text-high');
if (ratingEl) {
  const scoreTxt = ratingEl.textContent.trim(); // "5,0/5"
  const match = scoreTxt.match(/([\d.,]+)\s*\/\s*5/);
  if (match) {
    review_score = parseFloat(match[1].replace(',', '.')) || 0;
  }
}

// review count
const reviewsEl = document.querySelector('[data-test="rating-suffix"]');
if (reviewsEl) {
...
// Initialize popover on hover only
$(document).ready(function () {
  const $score = $('#popoverOption');

  $score.on('mouseenter', function () {
    // create + show a new popover on the left
    $(this).popover({
      html: true,
      placement: 'left'
    }).popover('show');
  ...
  $score.on('mouseleave', function () {
    // fully destroy popover when leaving
