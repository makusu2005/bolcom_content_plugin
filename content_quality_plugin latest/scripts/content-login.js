setTimeout(() => {
      ...
      for (const s of scripts) {
        ...
        for (const obj of objs) {
          ...
          if (Array.isArray(obj.hasVariant)) {
            const found = obj.hasVariant.find(
              (v) => v.gtin13 || v.ean || v.gtin
            );
  ...
  card.classList.add("login_card", "mb-3");
  ...
  links.style.gridAutoColumns = "1fr";
  const linksWrapper = document.createElement("div");
  linksWrapper.classList.add("links_wrapper");
  links.appendChild(linksWrapper);
  title.insertAdjacentElement("beforebegin", links);
  linksWrapper.appendChild(sdd_open);
  linksWrapper.appendChild(prod_open);

  links.insertAdjacentElement("beforeend", card);
  card.innerHTML =
    "<div >You're currently not logged in, please <a class='login' style='padding-block: 4px;' href='https://attalosagency.com/user-login/' target='_blank'>login</a> to see the Attalos content score of this product.</div>";
  ...
  removeBadTooltips();
  new MutationObserver(removeBadTooltips).observe(document.body, {
    childList: true,
    subtree: true,
  });
}, 4000);
