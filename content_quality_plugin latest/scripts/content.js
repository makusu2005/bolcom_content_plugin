setTimeout(() => {
  chrome.storage.sync.get(["on"]).then((result) => {
    let ean = null; // 👈 global, accessible everywhere
    let jsonData = null;
    ...
    if (jsonLdScript) {
      try {
        ...
        if (!ean && Array.isArray(jsonData.workExample)) {
          for (const example of jsonData.workExample) {
            if (Array.isArray(example.identifier)) {
              const found = example.identifier.find(
                (id) =>
                  id.propertyID &&
                  ["gtin13", "ean", "gtin"].includes(
                    id.propertyID.toLowerCase()
                  ) &&
                  id.value
              );
              if (found) {
        ...
        if (!ean && Array.isArray(jsonData.identifier)) {
          const found = jsonData.identifier.find(
            (id) =>
              id.propertyID &&
    ...
    //   '[class="fluid-grid fluid-grid--middle"]'
    // );

    var breadcrumbs = document.querySelector(
      '[class="flex items-center ps-none text-neutral-text-high [&_span>svg]:size-[1.125rem]"]'
    );
    var video = document.querySelector(
      '[class="snap-start list-none shrink-0"] svg'
    );

    var images = document.querySelectorAll(
      '[class="snap-start list-none shrink-0"]'
    );
    ...
    const ratingEl = document.querySelector(
      '[class="flex w-fit items-center justify-around gap-1"]>span'
    );

    if (ratingEl) {
      const scoreTxt = ratingEl.textContent.trim(); // "5,0/5"
      const match = scoreTxt.match(/\d+\,\d+/g);
      if (match) {
        review_score = parseFloat(match[0].replace(",", ".")) || 0;
      }
    }

    // review count
    const reviewsEl = document.querySelector(
      ".temporary-inline-button-fix.gap-1.align-bottom.relative.inline-flex.items-center.underline-offset-4"
    );
    if (reviewsEl) {
      const countTxt = reviewsEl.textContent.trim(); // "Bekijk 3 reviews"
      const countMatch = countTxt.match(/\d+/);
      if (countMatch) {
        no_reviews = Number(countMatch[0]) || 0;
      }
    }
    ...
    var title = document.querySelector(
      '[class="mb-3 typography-heading-primary-maximal-200 md:typography-heading-primary-maximal-400"]'
    );

    var title_text = document.querySelector(
      '[class="mb-3 typography-heading-primary-maximal-200 md:typography-heading-primary-maximal-400"]'
    );
    if (title_text) {
    ...
    if (switchStatus) {
      ...
      var main_imageEl = document.querySelector(
        '[class="min-h-full max-w-full max-h-full object-contain h-auto w-auto"]'
      );

      var main_image = main_imageEl?.getAttribute("src");
      ...
      pic.onload = function () {
        ...
        var bottomright = c.getImageData(
          pic.width - 1,
          pic.height - 1,
          1,
          1
        ...
        if (pic.width < 549 || pic.height < 549) {
          //points *= 0.9;
          image_size_check = good;
        }
        ...
        if (true) {
          ...
          // ✅ Add this once at the top of your UI injection code
        ...
        `;
          ...
          sdd_open.classList.add("attalos_links");
          sdd_open.style.marginRight = "12px";
          sdd_open.textContent = "SDD productoverzicht";
          sdd_open.id = "sdd_open";

          sdd_open.href =
            "https://attalosagency.com/out/index.php?url=https://partner.bol.com/sdd/product-overview/products/" +
            globalid;
          sdd_open.target = "_blank";

          const prod_open = document.createElement("a");
          prod_open.type = "button";
          prod_open.classList.add("attalos_links");
          prod_open.textContent = "SDD productcontent";
          prod_open.id = "prod_open";

          prod_open.href =
            "https://attalosagency.com/out/index.php?url=https://partner.bol.com/sdd/product-content/product/" +
            ean;
          ...
          col1.classList.add("attalos_score");
        ...
        <tr>
        <td>Good image quality<br /><small>current size: ${pic.height}px x ${pic.width}px</small></td>
        ...
        <td style="float:right">${review_score_check}</td>
          ...
          linksWrapper.classList.add("wrapper");
          title.insertAdjacentElement("beforebegin", linksWrapper);
          ...
          function removeBadTooltips() {
            document
              .querySelectorAll(".tooltip.js_tooltip_content")
              .forEach((el) => {
          ...
          linksWrapper.style.position = "relative";
          const cardEl = document.createElement("div");
          cardEl.classList.add("card_container");
          cardEl.style.backgroundColor = "white";
          cardEl.innerHTML = card.dataset.content;
          linksWrapper.appendChild(cardEl);

          linksWrapper.addEventListener("mouseenter", () => {
      ...
      if (breadcrumbsLinks.length > 0) {
        ...
        const lastBreadcrumbsLink =
          breadcrumbsLinks[breadcrumbsLinks.length - 1];
        // Split the URL by '/'
      ...
      chrome.runtime.sendMessage(
        { operation: "getSpotValue", ean: ean, category: category },
        (response) => {
